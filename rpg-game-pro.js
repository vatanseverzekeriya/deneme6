// Professional RPG Game with High-Quality Sprite Renderers
// Converted from emoji-based to professional pixel art assets

// Character Classes with renderer mapping
const CLASSES = {
    warrior: {
        name: 'Savaşçı',
        icon: '🛡️',
        rendererClass: 'WarriorRenderer',
        baseHP: 150,
        baseMP: 50,
        baseDamage: 15,
        baseDefense: 10,
...
// Mob types with renderer mapping
const MOB_TYPES = [
    { name: 'Kurt', icon: '🐺', rendererClass: 'WolfRenderer', hp: 50, damage: 8, xp: 25, gold: 10, speed: 1.5 },
    { name: 'Goblin', icon: '👹', rendererClass: 'GoblinRenderer', hp: 60, damage: 10, xp: 30, gold: 15, speed: 1.2 },
    { name: 'Ork', icon: '👾', rendererClass: 'OrcRenderer', hp: 80, damage: 12, xp: 40, gold: 20, speed: 1.0 },
    { name: 'Troll', icon: '🧟', rendererClass: 'TrollRenderer', hp: 120, damage: 15, xp: 60, gold: 30, speed: 0.8 },
    { name: 'Ejderha', icon: '🐉', rendererClass: 'DragonRenderer', hp: 200, damage: 25, xp: 100, gold: 50, speed: 0.6 }
];
...
class Game {
    ...
    // Initialize renderer for a class
    getRenderer(className) {
        if (!this.renderers[className]) {
            switch (className) {
                case 'WarriorRenderer':
                    this.renderers[className] = new WarriorRenderer();
                    break;
                case 'NinjaRenderer':
                    this.renderers[className] = new NinjaRenderer();
                    break;
                case 'ShamanRenderer':
                    this.renderers[className] = new ShamanRenderer();
                    break;
                case 'SuraRenderer':
                    this.renderers[className] = new SuraRenderer();
                    break;
                case 'WolfRenderer':
                    this.renderers[className] = new WolfRenderer();
                    break;
                case 'GoblinRenderer':
                    this.renderers[className] = new GoblinRenderer();
                    break;
                case 'OrcRenderer':
                    this.renderers[className] = new OrcRenderer();
                    break;
                case 'TrollRenderer':
                    this.renderers[className] = new TrollRenderer();
                    break;
                case 'DragonRenderer':
                    this.renderers[className] = new DragonRenderer();
                    break;
            }
        }
        return this.renderers[className];
    }
    ...
    selectCharacter(className) {
        ...
        // Create renderer for player
        this.playerRenderer = this.getRenderer(classData.rendererClass);
    ...
    spawnMob() {
        ...
        const mob = {
            ...
            hurtTimer: 0,
            isDead: false,
            renderer: this.getRenderer(type.rendererClass)
        };
    ...
    useSkill(index) {
        ...
        // Set skill animation flag
        this.playerUsingSkill = true;
        setTimeout(() => { this.playerUsingSkill = false; }, 500);
        ...
        if (skill.damage) {
            ...
            if (nearestMob) {
                ...
                if (distance < 300) {
                    ...
                    if (skill.name.includes('Yıldırım')) {
                        this.particleSystem.createLightningParticles(
                            this.player.x, this.player.y,
                            nearestMob.x, nearestMob.y
                        );
                        this.particleSystem.createSparkParticles(nearestMob.x, nearestMob.y, 20, '#00FFFF');
                    } else if (skill.name.includes('Işın')) {
                        this.particleSystem.createMagicParticles(nearestMob.x, nearestMob.y, 15, '#9370DB');
                    } else if (skill.name.includes('Savaş')) {
                        this.particleSystem.createExplosionParticles(nearestMob.x, nearestMob.y, 25, 0.8);
                    } else {
                        this.particleSystem.createSparkParticles(nearestMob.x, nearestMob.y, 15);
                    }
                    ...
                    if (skill.lifesteal) {
                        ...
                        this.particleSystem.createHealParticles(this.player.x, this.player.y, 10);
                    }
                }
            }
        }
        ...
        if (skill.heal) {
            ...
            this.particleSystem.createHealParticles(this.player.x, this.player.y, 20);
        }
    ...
    damageEnemy(enemy, damage) {
        ...
        this.particleSystem.createBloodParticles(enemy.x, enemy.y, 8);
    ...
    killEnemy(enemy) {
        ...
        // Death particles
        this.particleSystem.createSmokeParticles(enemy.x, enemy.y, 10);
    ...
    levelUp() {
        ...
        this.particleSystem.createMagicParticles(this.player.x, this.player.y, 50, '#FFD700');
        this.updateHUD();
    }
    ...
    pickupDrop(drop) {
        ...
        for (let i = 0; i < this.inventory.length; i++) {
            if (!this.inventory[i]) {
                ...
                this.particleSystem.createSparkParticles(this.player.x, this.player.y, 10, '#FFD700');
                break;
            }
        }
    }
    ...
    useItem(slot) {
        ...
        if (item.type === 'potion') {
            if (item.heal) {
                ...
                this.particleSystem.createHealParticles(this.player.x, this.player.y, 15);
            }
            if (item.mana) {
                this.player.mp = Math.min(this.player.maxMP, this.player.mp + item.mana);
                this.particleSystem.createMagicParticles(this.player.x, this.player.y, 15, '#4444FF');
            }
    ...
    update() {
        ...
        if (this.player.isHurt) {
            this.player.hurtTimer -= this.deltaTime;
        ...
        this.mobs.forEach(mob => {
            if (mob.isDead) {
                mob.renderer.update(this.deltaTime, false, false, false, true);
                return;
            }
            ...
            if (dist < 400) {
                ...
                mob.dx = Math.cos(angle) * mob.speed;
                mob.dy = Math.sin(angle) * mob.speed;
                mob.x += mob.dx;
                mob.y += mob.dy;
                ...
                if (dist < 50) {
                    if (mob.targetCooldown <= 0) {
                        ...
                        this.player.hurtTimer = 300;

                        this.showDamage(this.player.x, this.player.y - 40, damage);
                        this.particleSystem.createBloodParticles(this.player.x, this.player.y, 6);
            ...
            if (mob.targetCooldown > 0) {
                mob.targetCooldown -= this.deltaTime;
            }
        });
        ...
        this.player.skills.forEach(skill => {
            if (skill.cooldownRemaining > 0) {
                skill.cooldownRemaining -= this.deltaTime;
            }
        });
    ...
    draw() {
        ...
        // Mobs (render with new system)
        this.mobs.forEach(mob => {
            if (mob.renderer) {
                mob.renderer.render(this.ctx, mob.x, mob.y, mob.dx, mob.dy);
            }
        ...
        // Player (render with new system)
        if (this.player && this.playerRenderer) {
            this.playerRenderer.render(this.ctx, this.player.x, this.player.y, this.player.dx, this.player.dy);
        }
